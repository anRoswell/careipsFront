/**
 * @name CorregimientoController
 * @type Class
 * @description Controlador encargado de la validación de usuarios en el sistema
 * @author enosReyes
 */

const { CorregimientoService } = require('../../services/corregimiento.service')
const { Process } = require('../../base/process')
const Message = require('../../messages/message')
const Log4js = require('../../base/log4js')()
let _service = null
const table = 'M_Corregimientos'

class CorregimientoController {
	constructor() {
		_service = new CorregimientoService()
	}
	...
	async create(req, res) {
		try {
			...
			Log4js.error(`[action: CorregimientoController create][msg: ${error.message}][file:${__filename}]`)
	...
	async update(req, res) {
		try {
			...
			Log4js.error(`[action: CorregimientoController update][msg: ${error.message}][file:${__filename}]`)
	...
	async delete(req, res) {
		try {
			...
			Log4js.error(`[action: CorregimientoController delete][msg: ${error.message}][file:${__filename}]`)
	...
	async one(req, res) {
		try {
			...
			Log4js.error(`[action: CorregimientoController one][msg: ${error.message}][file:${__filename}]`)
	...
	async all(req, res) {
		try {
			...
			Log4js.error(`[action: CorregimientoController all][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
	...
	async list(req, res) {
		try {
			...
			Log4js.error(`[action: CorregimientoController list][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
}

module.exports.CorregimientoController = CorregimientoController
