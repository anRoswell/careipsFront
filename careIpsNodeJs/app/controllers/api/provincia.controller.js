...
let _service = null
const table = 'M_Provincias'

class ProvinciaController {
	constructor() {
		_service = new ProvinciaService()
	}
	...
	async create(req, res) {
		try {
			...
			Log4js.error(`[action: ProvinciaController create][msg: ${error.message}][file:${__filename}]`)
	...
	async update(req, res) {
		try {
			...
			Log4js.error(`[action: ProvinciaController update][msg: ${error.message}][file:${__filename}]`)
	...
	async delete(req, res) {
		try {
			...
			Log4js.error(`[action: ProvinciaController delete][msg: ${error.message}][file:${__filename}]`)
	...
	async one(req, res) {
		try {
			...
			Log4js.error(`[action: ProvinciaController one][msg: ${error.message}][file:${__filename}]`)
	...
	async all(req, res) {
		try {
			...
			Log4js.error(`[action: ProvinciaController all][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
	...
	async list(req, res) {
		try {
			...
			Log4js.error(`[action: ProvinciaController list][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
}

module.exports.ProvinciaController = ProvinciaController
