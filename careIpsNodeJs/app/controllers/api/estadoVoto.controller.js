/**
 * @name EstadoVotoController
 * @type Class
 * @description Controlador encargado de la validación de usuarios en el sistema
 * @author anRoswell
 */

const { EstadoVotoService } = require('../../services/estadoVoto.service')
const { Process } = require('../../base/process')
const Message = require('../../messages/message')
const Log4js = require('../../base/log4js')()
let _service = null
const table = 'M_EstadosVotos'

class EstadoVotoController {
	constructor() {
		_service = new EstadoVotoService()
	}
	...
	async create(req, res) {
		try {
			...
			Log4js.error(`[action: EstadoVotoController create][msg: ${error.message}][file:${__filename}]`)
	...
	async update(req, res) {
		try {
			...
			const { affectedRows } = await _service.update(fields, id, table)
			if (affectedRows) {
			...
			Log4js.error(`[action: EstadoVotoController update][msg: ${error.message}][file:${__filename}]`)
	...
	async delete(req, res) {
		try {
			const id = req.params.id
			const { affectedRows } = await _service.delete(id, table)
			if (affectedRows) {
			...
			Log4js.error(`[action: EstadoVotoController delete][msg: ${error.message}][file:${__filename}]`)
	...
	async one(req, res) {
		try {
			...
			if (data) {
				...
				return Process.error(res, Message('ID_NOT_EXISTS'))
			}
		} catch (error) {
			Log4js.error(`[action: EstadoVotoController one][msg: ${error.message}][file:${__filename}]`)
	...
	async all(req, res) {
		try {
			...
			Log4js.error(`[action: EstadoVotoController all][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
	...
	async list(req, res) {
		try {
			...
			Log4js.error(`[action: EstadoVotoController list][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
}

module.exports.EstadoVotoController = EstadoVotoController
