...
let _service = null
const table = 'mSliders'

class SliderController {
	constructor() {
		_service = new SliderService()
	}
	...
	async create(req, res) {
		try {
			...
			Log4js.error(`[action: SliderController create][msg: ${error.message}][file:${__filename}]`)
	...
	async update(req, res) {
		try {
			...
			Log4js.error(`[action: SliderController update][msg: ${error.message}][file:${__filename}]`)
	...
	async delete(req, res) {
		try {
			...
			Log4js.error(`[action: SliderController delete][msg: ${error.message}][file:${__filename}]`)
	...
	async one(req, res) {
		try {
			...
			Log4js.error(`[action: SliderController one][msg: ${error.message}][file:${__filename}]`)
	...
	async all(req, res) {
		try {
			const fields = ['title', 'mensaje', 'imageUrl', 'estado']
			const order = ['id']
			const data = await _service.all(fields, {}, order, table)
			return Process.success(res, data)
		} catch (error) {
			Log4js.error(`[action: SliderController all][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
	...
	async list(req, res) {
		try {
			...
			Log4js.error(`[action: SliderController list][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}

	async spXXXXXXX(req, res) {
		try {
		} catch (error) {
			Log4js.error(`[action: SliderController spAsignarPersonal][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
}
