/**
 * @name CentroVotacionController
 * @type Class
 * @description Controlador encargado de la validación de usuarios en el sistema
 * @author enosReyes
 */

const { CentroVotacionService } = require('../../services/centrovotacion.service')
const { Process } = require('../../base/process')
const Message = require('../../messages/message')
const Log4js = require('../../base/log4js')()
let _service = null
const table = 'M_CentroVotaciones'

class CentroVotacionController {
	constructor() {
		_service = new CentroVotacionService()
	}
	...
	async create(req, res) {
		try {
			...
			Log4js.error(`[action: CentroVotacionController create][msg: ${error.message}][file:${__filename}]`)
	...
	async update(req, res) {
		try {
			...
			Log4js.error(`[action: CentroVotacionController update][msg: ${error.message}][file:${__filename}]`)
	...
	async delete(req, res) {
		try {
			...
			Log4js.error(`[action: CentroVotacionController delete][msg: ${error.message}][file:${__filename}]`)
	...
	async one(req, res) {
		try {
			...
			Log4js.error(`[action: CentroVotacionController one][msg: ${error.message}][file:${__filename}]`)
	...
	async all(req, res) {
		try {
			...
			Log4js.error(`[action: CentroVotacionController all][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
	...
	async list(req, res) {
		try {
			...
			Log4js.error(`[action: CentroVotacionController list][msg: ${error.message}][file:${__filename}]`)
			return Process.error(res, Message('DATA_ERROR'))
		}
	}
}

module.exports.CentroVotacionController = CentroVotacionController
